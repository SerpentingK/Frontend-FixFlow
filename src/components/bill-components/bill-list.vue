...
const loadAllBills = async () => {
  try {
    ...
    const answer = await axios.get(
      `http://127.0.0.1:8089/someDataOfBill/${loggedCompany.value}`
    );
...
const searchBills = debounce(async () => {
  ...
  try {
    ...
    if (searchType.value === "1") {
      ...
      url = `http://127.0.0.1:8089/searchBillsByNumber/${loggedCompany.value}/${search.value}`;
    } else if (searchType.value === "2") {  
      // Filtrar por fecha
      url = `http://127.0.0.1:8089/searchBillsByDate/${loggedCompany.value}/${search.value}`;
    } else if (searchType.value === "3") {
      // Filtrar por cliente
      url = `http://127.0.0.1:8089/searchBillsByName/${loggedCompany.value}/${search.value}`;
    }
